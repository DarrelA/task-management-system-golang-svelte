<script>
    ...
    export async function handleSubmit(event) {
      ...
      console.log(json)
      try {
        const response = await axios.post(`http://localhost:4000/create-plan?AppAcronym=${appacronym}`, json, {withCredentials: true});
        if (response) {
    ...
    <Row>
        <Col>
            <FormGroup>
                ...
                <Input type="text" bind:value={plan_name} placeholder="Enter Plan Name" autofocus />
            </FormGroup>
        </Col>
        <Col>
            <FormGroup>
                <Label>Assign a Plan Color::</Label>
                <Input type="color" bind:value={plan_color} />
            </FormGroup>
        </Col>
    </Row>
    <Row>
